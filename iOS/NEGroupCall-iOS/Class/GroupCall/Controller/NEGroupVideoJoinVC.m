...
@property(nonatomic, strong) NEMultipleVideoNavView *navView;
@property (nonatomic,strong) NESetupSwitchView *cameraSwitchView;
@property (nonatomic,strong) NESetupSwitchView *microphoneSwitchView;
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [NEChannelSetupService sharedService].resolutionRatio = @"1280*720";
    [NEChannelSetupService sharedService].frameRate = @"30";
    [NEChannelSetupService sharedService].scenarioType = 2;
    [NEChannelSetupService sharedService].soundQuality = @"高清";
}

- (void)ne_initializeConfig {
    self.isCameraOpen = YES;
    self.isMicrophoneOpen = YES;
}
...
- (void)ne_addSubViews {
    ...
    [self.view addSubview:self.cameraSwitchView];
    [self.view addSubview:self.microphoneSwitchView];
    ...
    [self.cameraSwitchView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.view);
        make.height.mas_equalTo(56);
        make.top.equalTo(self.nickTextField.mas_bottom).offset(20);
    }];
    
    [self.microphoneSwitchView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.right.equalTo(self.view);
        make.height.mas_equalTo(56);
        make.top.equalTo(self.cameraSwitchView.mas_bottom);
    }];
...
- (void)ne_bindViewModel {
    ...
    [self.navView.backSubject subscribeNext:^(NSNumber *selectIndex) {
        @strongify(self)
        [self.navigationController popViewControllerAnimated:YES];
    }];
    
    self.navView.setupSubject = [RACSubject subject];
    [self.navView.setupSubject subscribeNext:^(id  _Nullable x) {
        @strongify(self)
        NEMultiCallSetupViewController *setUpCtrl = [[NEMultiCallSetupViewController alloc] init];
        NEActionSheetNavigationController *nav = [[NEActionSheetNavigationController alloc] initWithRootViewController:setUpCtrl];
        nav.dismissOnTouchOutside = YES;
        [self presentViewController:nav animated:YES completion:nil];
    }];
    
    self.cameraSwitchView.switchChangeSubject = [RACSubject subject];
    [self.cameraSwitchView.switchChangeSubject subscribeNext:^(NSNumber *ison) {
        @strongify(self)
        self.isCameraOpen = ison.boolValue;
    }];
    
    self.microphoneSwitchView.switchChangeSubject = [RACSubject subject];
    [self.microphoneSwitchView.switchChangeSubject subscribeNext:^(NSNumber *ison) {
        @strongify(self)
        self.isMicrophoneOpen = ison.boolValue;
    }];
...
- (NEMultipleVideoNavView *)navView {
    if (!_navView) {
        _navView = [[NEMultipleVideoNavView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, KNavBottom)];
    }
    return _navView;
}

- (NESetupSwitchView *)cameraSwitchView {
    if (!_cameraSwitchView) {
        _cameraSwitchView = [[NESetupSwitchView alloc]initWithtitle:@"入会时打开摄像头" switchStatus:YES];
    }
    return _cameraSwitchView;
}

- (NESetupSwitchView *)microphoneSwitchView {
    if (!_microphoneSwitchView) {
        _microphoneSwitchView = [[NESetupSwitchView alloc]initWithtitle:@"入会时打开麦克风" switchStatus:YES];
    }
    return _microphoneSwitchView;
}
@end
