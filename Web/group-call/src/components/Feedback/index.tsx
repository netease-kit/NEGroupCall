...
const Feedback: React.FC<IProps> = ({
    ...
    if (feedbackType === 0) {
      return (
        <div className={styles.badContent}>
          <div className={styles.badTitle}>很抱歉给您带来不便~请问您遇到了什么问题？</div>
          <MyCheckBoxGroup className={styles.badContentType} value={contentType} options={contentTypes} onChange={(value) => { setContentType(value as number[]) }} />
          <MyTextArea maxLength={1000} placeholder='其他问题...' value={content} onChange={(e) => { setContent(e.target.value) }} />
          <div className={styles.badFooter}>
            <MyButton type='primary' onClick={handleOnOk}>提交</MyButton>
          </div>
        </div>
      )
    }
  ...
  return (
    <MyModal
      ...
      maskClosable={false}
    >
      <MyRadioGroup value={feedbackType} onChange={(e) => { setFeedbackType(e.target.value) }}>
        <MyRadio value={1}>
