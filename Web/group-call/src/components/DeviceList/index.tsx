...
const DeviceList: React.FC<IProps> = ({
  data,
  postion,
  visible = true,
}) => {
  ...
  return visible ? (
    <div onClick={handleClickEmpty} style={{...postion}} className={styles.devieListWrapper}>
      {data.map(item => (
        <div key={item.title + item.type} className={styles.dataItem}>
          <div className={styles.title}>{item.title}</div>
          <div>
            {item.list.map(l => (
              <div onClick={() => {item.onChange({...l})}} key={l.value} className={styles.listItem}>
                <span className={styles.itemText}>{l.label}</span>
                {l.value === item.value && <Icon type='iconcheck-line-regular1x' width='12' height='12' color='#fff' />}
              </div>
            ))}
          </div>
        </div>
